#define WgammaPreSelector_cxx
#include "WgammaPreSelector.h"
#include "LinkDef.h"
...
void WgammaPreSelector::Loop(string outputFileName, float mcWeight) {
  ...
  float phoEtaMax                    =   2.44 ;
  float jetEtaMax                    =   2.44 ;
  //float jetT2T1Max                   =   0.5 ;
  float phoEtaRanges[5]              = {0, 0.75, 1.479, 2.4, 3.0};// Isn't used
  float leadingPhoWJetDeltaRCut      = 1.1;
  float WJetMinPtCut                 = 200;
  ...
  cout << "\n\nStarting WgammaPreSelector::Loop().\n" << endl;
  ...
  for (Long64_t jentry=0; jentry<nentries;++jentry) {
   ...
   // WJet_HbbTag                  = -999. ;
    ...
    for (uint iPh = 0; iPh<ph_pt->size() ; ++iPh) { 
      ...
      if (ph_mvaCat->at(iPh)==0){
         if(ph_mvaVal->at(iPh)>=0.68) eventsBPassingmva80++;
         if(ph_mvaVal->at(iPh)>=0.2) eventsBPassingmva90++;
      }
      if (ph_mvaCat->at(iPh)==1){
         if(ph_mvaVal->at(iPh)>=0.6) eventsEPassingmva80++;
         if(ph_mvaVal->at(iPh)>=0.2) eventsEPassingmva90++;
      }
    ...
    for (uint iJet = 0; iJet<jetAK8_puppi_pt->size() ; ++iJet) { 
      ...
      //Jet Cuts
      if (jetAK8_puppi_pt->at(iJet) > prevJetPt && jetAK8_IDTight->at(iJet) == 1 && jetAK8_IDTightLepVeto->at(iJet) == 1 && jetAK8_puppi_pt->at(iJet)>WJetMinPtCut && tmpLeadingJet.DeltaR(leadingPhoton) >= leadingPhoWJetDeltaRCut ) { 

        if (debugFlag && dumpEventInfo) cout << " this event PASSED the W Jet requirements! " << endl;
        prevJetPt = jetAK8_puppi_pt->at(iJet);
      ...
      else if (debugFlag && dumpEventInfo) cout << " this event failed the jet requirement for the W branch (or was a lower pT than previous Jet!" << endl;
    }
  ...
  cout << "Photon Passing Rate in Barrel:" << (float) eventsBPassingmva80/eventsBPassingmva90 << endl;
  cout << "Photon Passing Rate in Barrel:" << (float) eventsEPassingmva80/eventsEPassingmva90 << endl;
  cout << "\nCompleted output file is " << outputFileName.c_str() <<".\n" << endl;
}

float WgammaPreSelector::computeOverallSF(std::string category, float jetPt, float photonPt, float photonEta, bool debug) {
  return computePhotonSF(photonPt, photonEta, debug);
}

//TODO
float WgammaPreSelector::computePhotonSF(float photonPt, float photonEta, bool debug) {
  if (photonEta > 0) {
    if (photonEta < 0.8) {
