...
#ifndef WgammaPreSelector_h
#define WgammaPreSelector_h
...
class WgammaPreSelector {
public :
   TTree          *fChain;   //!pointer to the analyzed TTree or TChain
   Int_t           fCurrent; //!current Tree number in a TChain
  // TODO: Update with variables added in WgammaPreSelector
  ...
  int eventsBPassingmva80                 =    0; 
  int eventsBPassingmva90                 =    0; 
  int eventsEPassingmva80                 =    0;
  int eventsEPassingmva90                 =    0;
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   WgammaPreSelector(TTree *tree=0);
   virtual ~WgammaPreSelector();
...
#ifdef WgammaPreSelector_cxx
WgammaPreSelector::WgammaPreSelector(TTree *tree) : fChain(0) 
{
...
WgammaPreSelector::~WgammaPreSelector()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t WgammaPreSelector::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t WgammaPreSelector::LoadTree(Long64_t entry)
{
...
void WgammaPreSelector::Init(TTree *tree)
{
...
Bool_t WgammaPreSelector::Notify()
{
...
void WgammaPreSelector::Show(Long64_t entry)
{
...
Int_t WgammaPreSelector::Cut(Long64_t entry)
{
...
#endif // #ifdef WgammaPreSelector_cxx
