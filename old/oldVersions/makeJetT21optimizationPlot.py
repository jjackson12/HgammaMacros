...
dataFile = TFile("testSmall3data.root")
sigFile = TFile("testM750.root")
...
for i in range(30, 100):
    masscutSideband = TCut("(phJetInvMass_pruned_sideLowTwo>700)&&(phJetInvMass_pruned_sideLowTwo<800)&&(phJetDeltaR_sideLowTwo>0.8)")
    masscutData = TCut("(phJetInvMass_pruned_sig>700)&&(phJetInvMass_pruned_sig<800)&&(phJetDeltaR_sig>0.8)")
    cutValue = float(i)/float(100)
    thisSidebandCut = TCut("sideLowTwoJett2t1<%s"%str(cutValue))
    thisSigRegionCut = TCut("matchedJett2t1<%s"%str(cutValue))
    ...
    c = TCanvas()
    t = TGraph()
    data.Draw("matchedJett2t1", masscutData)
    ...
    sideLowTwo.Draw("sideLowTwoJett2t1", masscutSideband, "SAME")
    for primitive in c.GetListOfPrimitives():
    ...
    print primitives
    sig.Draw("matchedJett2t1", masscutData, "SAME")
    for primitive in c.GetListOfPrimitives():
        print primitive
...
c2 = TCanvas()
c2.cd()
g.SetTitle("S/#sqrt{B} for #tau_{21} cuts")
g.GetXaxis().SetTitle("max #tau_{21}")
g.GetYaxis().SetTitle("S/#sqrt{B}")
g.Draw()

outfile = TFile("optimization60to70_tau21.root", "RECREATE")
outfile.cd()
g.Write()

c2.Print("tau21optimization_60-70sideband.pdf")
