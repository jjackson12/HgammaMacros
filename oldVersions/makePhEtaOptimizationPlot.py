...
g = TGraph()
for i in range(3, 72):
    #masscutSideband = TCut("(phJetInvMass_pruned_sideLowTwo>700)&&(phJetInvMass_pruned_sideLowTwo<800)&&(phJetDeltaR_sideLowTwo>0.8)&&(sideLowTwoJet_pruned_abseta)<2.4)&&(cosThetaStar<0.7)")
    masscutSideband = TCut("(phJetInvMass_pruned_sideLowTwo>700)&&(phJetInvMass_pruned_sideLowTwo<800)&&(phJetDeltaR_sideLowTwo>0.8)&&(sideLowTwoJet_pruned_abseta<2.4)&&(cosThetaStar<0.7)")
    masscutData = TCut("(phJetInvMass_pruned_sig>700)&&(phJetInvMass_pruned_sig<800)&&(phJetDeltaR_sig>0.8)&&(matchedJet_pruned_abseta<2.4)&&(cosThetaStar<0.7)")
    cutValue = float(i)/float(30)
    thiscut = TCut("leadingPhAbsEta<%s"%str(cutValue))
    ...
    c = TCanvas()
    t = TGraph()
    data.Draw("leadingPhAbsEta", masscutData)
    ...
    sideLowTwo.Draw("leadingPhAbsEta", masscutSideband, "SAME")
    for primitive in c.GetListOfPrimitives():
    ...
    print primitives
    sig.Draw("leadingPhAbsEta", masscutData, "SAME")
    for primitive in c.GetListOfPrimitives():
        print primitive
...
outfile = TFile("optimization60to70_phEta.root", "RECREATE")
outfile.cd()
g.Write()

c2.Print("phEtaOptimization_60-70sideband.pdf")
